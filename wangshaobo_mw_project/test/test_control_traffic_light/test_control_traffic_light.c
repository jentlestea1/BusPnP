...
void print_traffic_light(void* t_node){
    ...
    for(;i<((traffic_light_repos*)p_node)->list_len;i++){
        ...
        uint_32 RT_priority=p_node->RT_section_priority[i];
        uint_32 dev_num=p_node->dev_num[i];
...
int main(){
    ...
    for(;num<len;num++){
        ...
        uint_32 RT_num=get_config_node_len(p);
        uint_32 i=0;
        printf("port: \n");
        for(;i<RT_num;i++){
    ...
    printf("打印配置信息结束...\n");
    printf("开始测试交通灯模块...\n");
    uint_32 con_id=get_1553_bus_config_id("001");
    void* pt= get_config_node(con_id);
    printf("config_id:%d\n",con_id);
    uint_32 traffic_repos_id=get_config_node_traffic_repos_id(pt);
    ...
    print_traffic_light(t_node);
    uint_32 scan1=get_scan_pos(traffic_repos_id);
    uint_32 scan2=get_scan_pos(traffic_repos_id);
    printf("scan1:%d scan2:%d\n",scan1,scan2);
    printf("2___\n");
    set_traffic_light(traffic_repos_id,"001",NOT_SET,LOADED,NOT_SET);
    traffic_repos_scan_func(traffic_repos_id);
    uint_32 scan3=get_scan_pos(traffic_repos_id);
    printf("scan3:%d\n",scan3);
    print_traffic_light(t_node);
    printf("3___\n");
    traffic_repos_scan_func(traffic_repos_id);
    uint_32 scan4=get_scan_pos(traffic_repos_id);
    printf("scan4:%d\n",scan4);
    print_traffic_light(t_node);
    printf("4___\n");
    traffic_repos_scan_func(traffic_repos_id);
    uint_32 scan5=get_scan_pos(traffic_repos_id);
    printf("scan5:%d\n",scan5);
    print_traffic_light(t_node);
    uint_32 scan6=get_scan_pos(traffic_repos_id);
    printf("scan6:%d\n",scan6);
    printf("5___\n");
    traffic_repos_scan_func(traffic_repos_id);
    print_traffic_light(t_node);
