...
#endif
#define PACKAGE_HEADER (uint_32)0xff0000ff
...
int frame_type_detect(struct can_frame frame,ubyte_8* w_buf,uint_32 *size,int size_frame_mask); //默认一帧为8\
        位,size为有效位,w_buf为如果是 数据帧则填入w_buf,type_mask可以屏蔽SIZE类型,
struct can_frame serial_frame(int serial_type,ubyte_8* r_buf,uint_32 size); //serial_type标示三种类型\
