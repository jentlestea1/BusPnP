...
int RT_send_package(ubyte_8 *buffer,uint_32 size){
    struct can_frame frame;
    frame=serial_frame(VCAN_SIZE_FRAME_FLAG,NULL,size);
    RT_send_frame(frame);
    sleep_ms(10);
    uint_32 frame_size_tmp;
    int i=0;
    for(i=0;i<size;){
...
uint_32 RT_receive_package(ubyte_8 *buffer){   //返回接收到的全帧大小,buffer大小满足情况
    int n;
    uint_32 pack_size;
    uint_32 size;
...
///*dev测试用*/
//void create_RT_socket_client(byte_8* buffer,uint_32 port){
//    int    sockfd, recv_len;
//    struct sockaddr_in    servaddr;  
//    ubyte_8 recv_buffer[4096];
//    uint_32 size;
...
void* create_RT_ret_socket_client(void* RT_port){//以原port+1发
...
#endif
    sleep_ms(500);
    ubyte_8 ret_buff[4096]={0};
    uint_32 ret_size;
#ifdef __RT_TCPIP_TRANSMIT
    port_con* p_RT_port=(port_con*)RT_port;
    uint_32 port=p_RT_port->port+1;
    int sockfd,recv_len;  
    struct sockaddr_in  servaddr;
    //第一次要初始化RT端口
    if( (sockfd = socket(AF_INET, SOCK_STREAM, 0)) < 0){  
    ...
    init_port_array((uint_32 *)(ret_buff+8),*(uint_32 *)(ret_buff+4));
    close(sockfd);
    while(1){
        sleep_ms(30);
        if( (sockfd = socket(AF_INET, SOCK_STREAM, 0)) < 0){  
    ...
    uint_32 port_len=*(uint_32 *)ret_buff;
    port_len=htonl(port_len);
    init_port_array((uint_32 *)(ret_buff+sizeof(uint_32)),port_len);
    //初始化完成之后再创建RT_server
    pthread_mutex_lock(&RT_recv_mutex);
...
/*void create_RT_unit(uint_32 port,uint_32 child_port){
...
void initialize_RT(void){
    ...
    int i=0;
    uint_32 port_len=*(uint_32 *)(buff+4);
    uint_32 *p=(uint_32 *)(buff+8);
