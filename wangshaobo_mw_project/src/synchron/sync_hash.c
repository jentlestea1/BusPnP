...
uint_32 Hash(HASH_TYPE hash_type,HASHKEY key){
    //求出下标
...
void insert_hash_node(uint_32 pos,HASHKEY key,void *p){
    node *prev=NULL;
...
void *get_hash_value(HASH_TYPE hash_type,uint_32 d1,uint_32 d2,byte_8 *s1){
    HASHKEY key=0;
    key=get_hash_key(hash_type,d1,d2,s1); 
    uint_32 pos=Hash(hash_type,key);
    node *prev=NULL;
    if(table.node_head[pos]!=NULL){
        prev=table.node_head[pos];
        while(prev!=NULL){
